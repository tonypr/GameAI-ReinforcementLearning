...
class Connect4Game(Game):
    def __init__(self):
        ...
        self.w = 7
        self.h = 6
        self.start = tuple(() for i in range(self.w))

    def actions(self, state):
        return [i+1 for i in range(self.w) if len(state[i]) < self.h]
    ...
    def check_square(self, state, x, y):
        ...
        for j in xrange(4):
            if x-j >= 0 and x-j+3 < self.w:
                horizontal_squares = tuple(state[x-j+i][y] for i in xrange(4) if len(state[x-j+i]) > y)
                if len(horizontal_squares) == 4:
                    square_states.add(horizontal_squares)

                if y-j >= 0 and y-j+3 < self.h:
                    diag_up_squares = tuple(state[x-j+i][y-j+i] for i in xrange(4) if len(state[x-j+i]) > y-j+i)
                    if len(diag_up_squares) == 4:
                        square_states.add(diag_up_squares)

                if y+j-3 >= 0 and y+j < self.h:
                    diag_down_squares = tuple(state[x-j+i][y+j-i] for i in xrange(4) if len(state[x-j+i]) > y+j-i)
                    if len(diag_down_squares) == 4:
                        square_states.add(diag_down_squares)

            if y-j >= 0 and y-j+3 < self.h:
                vertical_squares = tuple(state[x][y-j+i] for i in xrange(4) if len(state[x]) > y-j+i)
                if len(vertical_squares) == 4:
